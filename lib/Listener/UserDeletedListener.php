...
class UserDeletedListener implements IEventListener {
	...
	public function handle(Event $event): void {
        ...
        //$response = $this->aaochatService->sendWebhookData('userDeleted',$userData);
        $response = $this->aaochatService->deleteUserdataFromAaochat($userData);

        if($this->aaochatService->isAaochatApiLogEnable()) {
            $aaochat_log_dir = $this->aaochatService->getAaochatLogPath();
		    $userData = json_encode($userData);
            $myfile = file_put_contents($aaochat_log_dir.'userstatus_delete.txt', $userData.PHP_EOL , FILE_APPEND | LOCK_EX);
        }
